        ...
        .main-area(v-if="!isIE")
            .top-bar
                ul(v-if="isQuestionsLoaded")
                    li(v-for="(question, index) in questions" v-bind:key="(question, index)")
                        a(v-on:click="goToQuestion(index)" :id="index")
            ...
            template(v-if="isQuestionsLoaded")
                transition-group.answers-area(appear tag="div" name="custom-classes-transition" enter-active-class="answers-area-enter")
                    .answer-button(v-for="(answer, index) in questions[currentQuestionIndex].answers" v-on:click="submitAnswer" v-bind:key="`ab-${index}-${currentQuestionIndex}`" :id="index" v-bind:class=" { 'gradient-grey': isButtonGrey(currentQuestionIndex, index), 'gradient-green': isButtonGreen(currentQuestionIndex, index), 'gradient-orange' : isButtonRed(currentQuestionIndex, index) } ")
...
export default {
  data: () => ({
    backendUrl: isDev
      ? `http://192.168.1.224:81/mamochkagoda/online-game/`
  ...
  methods: {
    async loadStatistics () {
      if (this.discountCode === null && this.wrongAnswers !== 0) {
        let response = await axios.get(`${this.backendUrl}?game_mode=load_discount_code&secret=${this.secret}`)
    ...
    async loadNewQuestions () {
      let response = await axios.get(`${this.backendUrl}?game_mode=start_new_game`)
    ...
    async loadQuestionsFromCookieQuestionContext () {
      ...
      let response = await axios.get(`${this.backendUrl}?game_mode=continue_game&question_index=${ids.join(',')}`)
    ...
    async loadSecret () {
      let secret = await axios.get(`${this.backendUrl}?game_mode=generate_secret`)
    ...
    async processAnswer (event) {
      ...
      let response = await axios.get(`${this.backendUrl}?game_mode=load_correct_answer&question_index=${question.id}&secret=${this.secret}`)
